...
public class PlayerThread extends Thread {
    ...
    private ConfirmationThread confThread;
            ...
            while (running) {
                ...
                if (awaitingConfirmation) {
                    if (command.equals("CONFIRMED")) {
                        //System.out.println(player.getIndex()+" :: "+split[1]+" confirmed");
                        awaitingConfirmation = false;
                        confThread.confirm();
                        if (commandQueue.size( )> 0) {
                            sendCommand(commandQueue.remove());
                        }
                    } else {
                        System.out.println("Client out of sync");
